...
func add_action_script(name: String = "", path: String = "") -> void:
	if actions.find(func(a): return a.name == name) != -1:
		push_error("PARLEY_ERR: Action with name %s already exists" % [name])
		return
	var action_inst = load(path)
	if not action_inst:
		push_error("PARLEY_ERR: Action script does not exist at path: %s" % [path])
		return
	if not is_instance_of(action_inst, ActionInterface):
		push_error("PARLEY_ERR: Action script must inherit: ActionInterface")
		return
...
func get_action_by_id(id: int) -> Action:
	...
	if filtered_actions.size() == 0:
		...
		push_error("PARLEY_ERR: Action with id %s not found in store" % [id])
		return Action.new(-1)
...
func get_action_by_name(name: String) -> Action:
	...
	if filtered_actions.size() == 0:
		# TODO: is there a better way of handling this error here?
		push_error("PARLEY_ERR: Action with name %s not found in store" % [name])
		return Action.new(-1)
	return filtered_actions.front()
...
func get_action_by_ref(ref: String) -> Action:
	...
	if filtered_actions.size() == 0:
		# TODO: is there a better way of handling this error here?
		push_error("PARLEY_ERR: Action with ref %s not found in store" % [ref])
		return Action.new(-1)
	return filtered_actions.front()
