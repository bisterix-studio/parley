...
func add_action_script(name: String = "", path: String = "") -> void:
	if actions.find(func(a): return a.name == name) != -1:
		ParleyUtils.log.error("Action with name %s already exists" % [name])
		return
	var action_inst = load(path)
	if not action_inst:
		ParleyUtils.log.error("Action script does not exist at path: %s" % [path])
		return
	if not is_instance_of(action_inst, ActionInterface):
		ParleyUtils.log.error("Action script must inherit: ActionInterface")
		return
...
func get_action_by_id(id: int) -> Action:
	...
	if filtered_actions.size() == 0:
		...
		ParleyUtils.log.error("Action with id %s not found in store" % [id])
		return Action.new(-1)
