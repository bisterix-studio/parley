...
func _update_current_dialogue_ast(new_current_dialogue_ast: DialogueAst) -> void:
	...
	node_list.clear()
	filtered_nodes = []
	for node in current_dialogue_ast.nodes:
		var raw_node_string = str(node.to_dict())
		if not node_filter or raw_node_string.containsn(node_filter):
			node_list.add_item("%s [ID: %s]" % [DialogueAst.get_type_name(node.type), node.id])
...
func _update_dialogue_asts(updated_dialogue_asts: Array[DialogueAst]):
	dialogue_asts = updated_dialogue_asts
	if not dialogue_sequences_list:
		return
	dialogue_sequences_list.clear()
	filtered_dialogue_asts = []
	for dialogue_ast in dialogue_asts:
		var parts = (dialogue_ast.resource_path if dialogue_ast.resource_path else 'Unknown').split('/')
		var filename = parts[parts.size() - 1]
		if not dialogue_ast_filter or filename.containsn(dialogue_ast_filter):
			dialogue_sequences_list.add_item(filename)
...
func add_dialogue_ast(dialogue_ast: DialogueAst) -> void:
	...
	var filtered = dialogue_asts.filter(func(d: DialogueAst): return d.resource_path == dialogue_ast.resource_path)
